...
socket.on('updateConnections', player => {
    ...
    if(clientBalls[player.id] === undefined){
 	...
 	clientBalls[player.id] =  new Ball(player.x,  player.y, 25, 10);
        ...
        if(player.id === selfID){
            document.getElementById('playerWelcome').innerHTML =
                `enter your nickname and start to play (in room no.${player.roomNo})`
...
socket.on('updateFootball', footballPos => {
    if(football === undefined){
        football = new Ball(footballPos.x, footballPos.y, 10, 10);
        football.color = "red";
    } else {
